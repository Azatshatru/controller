            ...
            if(!$enquiry->errors())
            {
                if($detail = $this->Enquiries->save($enquiry))
                {
                    ...
                    $email->emailFormat('html')
                      ...
                      ->send();
                    $this->Flash->success(__('Volunteer form has been submitted successfully.'));
                    return $this->redirect(['action' => 'volunteer']);
					
                }
                else
                {
                    $this->Flash->error(__('Volunteer form could not be submitted. Please see warning(s) below.'));
                }
            }
            else
            {
        ...
        if($this->request->is('post'))
        {
            ...
            if(!$enquiry->errors())
            {
                ...
                if($this->request->getData('material_image.tmp_name'))
                {
                    ...
                    if(!$success)
                    {
                        $errors[] = __('Unable to upload  photo. Please try again.');
                    }
                }
				if(empty($errors))
                {
					if($detail = $this->Enquiries->save($enquiry))
					{
						...
						$email->emailFormat('html')
						  ...
						  ->send();
						$this->Flash->success(__('Donation form has been submitted successfully.'));
						return $this->redirect(['action' => 'donation']);
						
					}
					else
					{
						$this->Flash->error(__('Donation form could not be submitted. Please see warning(s) below.'));
					}
				}
                else
                {
